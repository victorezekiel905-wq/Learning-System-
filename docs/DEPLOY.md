...
3. **Apply the latest schema.** SQL Editor → New query → paste all of `supabase/updates/2026-09-24_production_release.sql` → Run. The last row should show `lockdown_ready`, `operations_ready` and `scale_ready` all `true`. It's safe to run twice. If you already ran it before 2026-09-25, also run `supabase/updates/2026-09-25_screen_channel_join.sql` (lets students join their own live-screen channel). `/api/health` should then show `"schema":"0770"`.
...
4. Deploy. Then open `https://your-domain/api/health`. It should return `{"ok":true,"db":"up","schema":"0770",...}`, and `missing_legal_details` should be `[]`.
...
- [ ] `2026-09-24_production_release.sql` applied; `/api/health` shows `"schema":"0770"`
